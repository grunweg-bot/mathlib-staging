...
theorem eq_zero_of_eval_zero_at_prod_finset {σ : Type*} [Finite σ] [IsDomain R]
  ...
  | @of_equiv σ τ e h =>
    suffices MvPolynomial.rename e.symm P = 0 by
      have that : Function.Injective (MvPolynomial.rename e.symm).toRingHom :=
        MvPolynomial.rename_injective (R := R) e.symm (e.symm.injective)
      rw [RingHom.injective_iff_ker_eq_bot] at that
      have this' : (MvPolynomial.rename e.symm).toRingHom P = 0 := this
