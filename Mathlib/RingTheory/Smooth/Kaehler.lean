...
-/
@[simps]
def derivationOfSectionOfKerSqZero (f : P →ₐ[R] S) (hf' : (RingHom.ker f.toRingHom) ^ 2 = ⊥)
    (g : S →ₐ[R] P) (hg : f.comp g = AlgHom.id R S) :
    Derivation R P (RingHom.ker f.toRingHom) where
  toFun x := ⟨x - g (f x), by
