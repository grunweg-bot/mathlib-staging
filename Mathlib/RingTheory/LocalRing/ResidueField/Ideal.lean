...
/-- If `f` sends `I` to `0` and `Iᶜ` to units, then `f` lifts to `κ(I)`. -/
noncomputable
def Ideal.ResidueField.liftₐ (I : Ideal A) [I.IsPrime] (f : A →ₐ[R] B)
    (hf₁ : I ≤ RingHom.ker f.toRingHom)
...
lemma Ideal.ResidueField.liftₐ_algebraMap (I : Ideal A) [I.IsPrime] (f : A →ₐ[R] B)
    (hf₁ : I ≤ RingHom.ker f.toRingHom) (hf₂ : I.primeCompl ≤ (IsUnit.submonoid B).comap f)
    (r : A) :
