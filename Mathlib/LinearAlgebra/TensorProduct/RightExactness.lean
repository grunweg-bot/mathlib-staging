...
lemma Algebra.TensorProduct.lTensor_ker (hg : Function.Surjective g) :
    RingHom.ker (map (AlgHom.id R A) g : A ⊗[R] C →+* A ⊗[R] D) =
      (RingHom.ker (g : C →+* D)).map (Algebra.TensorProduct.includeRight : C →ₐ[R] A ⊗[R] C) := by
  rw [← Submodule.restrictScalars_inj R]
  have : (RingHom.ker (map (AlgHom.id R A) g : A ⊗[R] C →+* A ⊗[R] D)).restrictScalars R =
...
lemma Algebra.TensorProduct.rTensor_ker (hf : Function.Surjective f) :
    RingHom.ker (map f (AlgHom.id R C) : A ⊗[R] C →+* B ⊗[R] C) =
      (RingHom.ker (f : A →+* B)).map (Algebra.TensorProduct.includeLeft : A →ₐ[R] A ⊗[R] C) := by
  rw [← Submodule.restrictScalars_inj R]
  have : (RingHom.ker (map f (AlgHom.id R C) : A ⊗[R] C →+* B ⊗[R] C)).restrictScalars R =
...
theorem Algebra.TensorProduct.map_ker (hf : Function.Surjective f) (hg : Function.Surjective g) :
    RingHom.ker (map f g : A ⊗[R] C →+* B ⊗[R] D) =
      (RingHom.ker (f : A →+* B)).map (Algebra.TensorProduct.includeLeft : A →ₐ[R] A ⊗[R] C) ⊔
        (RingHom.ker (g : C →+* D)).map
  ...
  rw [this, AlgHom.comp_toRingHom, ← RingHom.comap_ker]
  -- apply one step of exactness
  rw [← Algebra.TensorProduct.lTensor_ker _ hg,
    RingHom.ker_eq_comap_bot (map (AlgHom.id R A) g : A ⊗[R] C →+* A ⊗[R] D)]
  rw [← Ideal.comap_map_of_surjective (map (AlgHom.id R A) g : A ⊗[R] C →+* A ⊗[R] D)
